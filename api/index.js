...
const performSiteSync = async (userId, siteId, brandVariations = [], daysToFetch = 90) => {
    ...
    // Fetch back IDs
    const { data: savedKeywords, error: savedKwError } = await getSupabaseAdmin()
        .from('keywords')
        .select('id, keyword')
        .eq('site_id', siteId)
    if (savedKwError) throw savedKwError
    if (!savedKeywords) throw new Error('Failed to fetch keywords after upsert')
...
app.all('/api/cron/daily-sync', async (req, res) => {
